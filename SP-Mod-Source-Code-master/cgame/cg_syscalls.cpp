...
void	cgi_R_RenderScene( const refdef_t *fd ) {
#ifdef _XBOX
	const int xboxRenderSceneLog = (cg.time >= 3400 && cg.time <= 5000);
	if (xboxRenderSceneLog)
	{
		XBLF("JA: CL_EARLY EF cgi_R_RenderScene before syscall fd=%p time=%d vieworg=(%g,%g,%g) fov=(%g,%g) rd=0x%x",
			(void*)fd, fd ? fd->time : -1,
			fd ? fd->vieworg[0] : 0.0f, fd ? fd->vieworg[1] : 0.0f, fd ? fd->vieworg[2] : 0.0f,
			fd ? fd->fov_x : 0.0f, fd ? fd->fov_y : 0.0f, fd ? fd->rdflags : 0);
	}
#endif
	...
	if (xboxRenderSceneLog)
	{
		XBLog_Write("JA: CL_EARLY EF cgi_R_RenderScene after syscall");
	}
#endif
}
