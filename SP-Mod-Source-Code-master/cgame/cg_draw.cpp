...
{//ID teammates, ID enemies, ID objectives, etc.
	...
	rankCharIndex = raceCharIndex = htCharIndex = wtCharIndex = weapCharIndex = charIndex = floor((float)infoStringCount/33.0f);
...
void CG_DrawActive( stereoFrame_t stereoView ) {
	...
	if (xboxDrawActiveLog)
	{
		XBLF("JA: CL_EARLY EF CG_DrawActive enter time=%d stereo=%d snap=%p vieworg=(%g,%g,%g) fov=(%g,%g) rd=0x%x",
			cg.time, (int)stereoView, (void*)cg.snap,
			cg.refdef.vieworg[0], cg.refdef.vieworg[1], cg.refdef.vieworg[2],
			cg.refdef.fov_x, cg.refdef.fov_y, cg.refdef.rdflags);
	}
#endif
	...
	if ( !cg.snap ) {
#ifdef _XBOX
		if (xboxDrawActiveLog) XBLog_Write("JA: CL_EARLY EF CG_DrawActive no snap information path");
#endif
		CG_DrawInformation();
		return;
	}
...
#ifdef _XBOX
	if (xboxDrawActiveLog) XBLog_Write("JA: CL_EARLY EF CG_DrawActive before AngleVectors");
#endif
...
#ifdef _XBOX
	if (xboxDrawActiveLog) XBLog_Write("JA: CL_EARLY EF CG_DrawActive after AngleVectors");
#endif
...
#ifdef _XBOX
	if (xboxDrawActiveLog) XBLog_Write("JA: CL_EARLY EF CG_DrawActive before CG_TileClear");
#endif
	CG_TileClear();
#ifdef _XBOX
	if (xboxDrawActiveLog) XBLog_Write("JA: CL_EARLY EF CG_DrawActive after CG_TileClear");
#endif
