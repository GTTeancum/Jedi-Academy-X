...
void CalcEntitySpot ( gentity_t *ent, spot_t spot, vec3_t point ) 
{
	...
	case SPOT_WEAPON:
		if( !VectorCompare( NPCInfo->shootAngles, vec3_origin ) && !VectorCompare( NPCInfo->shootAngles, ent->client->ps.viewangles ))
		{
			AngleVectors( NPCInfo->shootAngles, forward, right, up );
		}
		else
		{
			AngleVectors( ent->client->ps.viewangles, forward, right, up );
		}
		CalcMuzzlePoint( ent, forward, right, up, point, 0 );
		//NOTE: automatically takes leaning into account!
		break;
...
qboolean NPC_UpdateAngles ( qboolean doPitch, qboolean doYaw ) 
{
	...
	if(NPCInfo->lookMode)
	{
		...
		yawSpeed *= (float)NPCInfo->lookMode/(float)LT_FULLFACE;
	}
...
//		NPCInfo->aimTime = level.time + 250;
	...
	{//We have a specific kind of looking we are doing,
		//lower the lookMod, slower the yawspeed
		yawSpeed *= (float)NPCInfo->lookMode/(float)LT_FULLFACE;
	}
...
gentity_t *NPC_PickEnemyExt( void )
{
...
qboolean NPC_FindEnemy( void )
{
	...
	gentity_t *newenemy = NPC_PickEnemyExt( );
...
qboolean NPC_CheckEnemyExt( void )
{
...
qboolean NPC_FaceEntity( gentity_t *ent, qboolean doPitch )
{
	...
	//Get the positions
	CalcEntitySpot( ent, SPOT_HEAD, entPos );
...
qboolean NPC_CheckDisguise( gentity_t *ent )
{
	if ( ent->client && ent->client->playerTeam == TEAM_DISGUISE )
		return qtrue;

	//NOTENOTE: DO NOT RE-ADD THE WEAPON CHECKING!!!!
