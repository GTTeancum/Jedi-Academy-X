...
{
#ifdef _XBOX
	int entNum = (int)(self - g_entities);
	qboolean xbLogNPC = (level.time <= 1200 || level.framenum <= 4);
	if (xbLogNPC)
	{
		XBLF("STEFX: NPC_Think enter ent=%d class='%s' health=%d npc=%p client=%p nextBState=%d",
			entNum,
			self && self->classname ? self->classname : "(null)",
			self ? self->health : -9999,
			self ? self->NPC : NULL,
			self ? self->client : NULL,
			(self && self->NPC) ? self->NPC->nextBStateThink : -1);
	}
#endif
	...
	if (xbLogNPC)
	{
		XBLF("STEFX: NPC_Think ent=%d after SetNPCGlobals NPC=%p NPCInfo=%p client=%p", entNum, NPC, NPCInfo, client);
	}
#endif
	...
	if ( debugNPCFreeze->value ) 
	{
		...
		if (xbLogNPC)
		{
			XBLF("STEFX: NPC_Think ent=%d debug freeze before ClientThink", entNum);
		}
#endif
		...
		if (xbLogNPC)
		{
			XBLF("STEFX: NPC_Think ent=%d debug freeze return", entNum);
		}
#endif
		return;
	}
	...
	if(!self || !self->NPC || !self->client)
	{
		...
		if (xbLogNPC)
		{
			XBLF("STEFX: NPC_Think ent=%d missing self/NPC/client", entNum);
		}
#endif
		return;
	}
	...
	if (self->health <= 0) 
	{
		...
		if (xbLogNPC)
		{
			XBLF("STEFX: NPC_Think ent=%d dead before DeadThink", entNum);
		}
#endif
		DeadThink();
#ifdef _XBOX
		if (xbLogNPC)
		{
			XBLF("STEFX: NPC_Think ent=%d dead after DeadThink", entNum);
		}
#endif
		if(NPCInfo->nextBStateThink <= level.time)
		{
			if( self->taskManager && !stop_icarus )
			{
				...
				if (xbLogNPC)
				{
					XBLF("STEFX: NPC_Think ent=%d dead before taskManager Update", entNum);
				}
#endif
				...
				if (xbLogNPC)
				{
					XBLF("STEFX: NPC_Think ent=%d dead after taskManager Update", entNum);
				}
#endif
			}
		}
	...
	if(NPCInfo->nextBStateThink <= level.time)
	{
		...
		if (xbLogNPC)
		{
			XBLF("STEFX: NPC_Think ent=%d bstate due before eType check eType=%d", entNum, NPC->s.eType);
		}
#endif
		/*
		if( self->taskManager && !stop_icarus )
		...
		*/
		...
		{//Something drastic happened in our script
			...
			if (xbLogNPC)
			{
				XBLF("STEFX: NPC_Think ent=%d bstate abort eType=%d", entNum, NPC->s.eType);
			}
#endif
			return;
		}
		...
		if (xbLogNPC)
		{
			XBLF("STEFX: NPC_Think ent=%d before NPC_ExecuteBState", entNum);
		}
#endif
		...
		if (xbLogNPC)
		{
			XBLF("STEFX: NPC_Think ent=%d after NPC_ExecuteBState", entNum);
		}
#endif
		...
		if( self->taskManager && !stop_icarus )
		{
			...
			if (xbLogNPC)
			{
				XBLF("STEFX: NPC_Think ent=%d before taskManager Update", entNum);
			}
#endif
			self->taskManager->Update( );
#ifdef _XBOX
			if (xbLogNPC)
			{
				XBLF("STEFX: NPC_Think ent=%d after taskManager Update", entNum);
			}
#endif
		}
	}
	else
	{
		...
		{//If we were following a roff, we don't do normal pmoves.
			...
			if (xbLogNPC)
			{
				XBLF("STEFX: NPC_Think ent=%d before NPC_UpdateAngles/ClientThink", entNum);
			}
#endif
			NPC_UpdateAngles(qtrue, qtrue);
			...
			if (xbLogNPC)
			{
				XBLF("STEFX: NPC_Think ent=%d after ClientThink", entNum);
			}
#endif
		}
		else
		{
			...
			if (xbLogNPC)
			{
				XBLF("STEFX: NPC_Think ent=%d before NPC_ApplyRoff", entNum);
			}
#endif
			NPC_ApplyRoff();
#ifdef _XBOX
			if (xbLogNPC)
			{
				XBLF("STEFX: NPC_Think ent=%d after NPC_ApplyRoff", entNum);
			}
#endif
		}
		VectorCopy(self->s.origin, self->s.origin2 );
	...
	if (xbLogNPC)
	{
		XBLF("STEFX: NPC_Think exit ent=%d nextthink=%d nextBState=%d", entNum, self->nextthink, self->NPC ? self->NPC->nextBStateThink : -1);
	}
#endif
}
