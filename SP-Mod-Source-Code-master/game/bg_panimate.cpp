...
int PM_AnimLength( int index, animNumber_t anim )
{
	...
	return knownAnimFileSets[index].animations[anim].numFrames * fabs((float)knownAnimFileSets[index].animations[anim].frameLerp);
}
...
*/
		...
		{//FIXME: allow to set a specific time?
			PM_SetTorsoAnimTimer( gent, torsoAnimTimer, animations[anim].numFrames * fabs((float)animations[anim].frameLerp) );
		}
	}
...
*/
		if ((gent->client) && (setAnimFlags & SETANIM_FLAG_HOLD))
		{//FIXME: allow to set a specific time?
			PM_SetLegsAnimTimer( gent, legsAnimTimer, animations[anim].numFrames * fabs((float)animations[anim].frameLerp) );
		}
	}
