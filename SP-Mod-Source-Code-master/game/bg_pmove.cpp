...
#ifdef _XBOX
static qboolean STEFX_PmoveVec3Bad(const vec3_t v)
{
	return (qboolean)(IS_NAN(v[0]) || IS_NAN(v[1]) || IS_NAN(v[2]));
}

static qboolean STEFX_PmoveFloatBad(float v)
{
	return (qboolean)(IS_NAN(v));
}

static qboolean STEFX_PmoveNearZero2D(const vec3_t v)
{
	return (qboolean)(v[0] > -0.1f && v[0] < 0.1f && v[1] > -0.1f && v[1] < 0.1f);
}

static qboolean STEFX_PmoveNearZero3D(const vec3_t v)
{
	return (qboolean)(v[0] > -0.1f && v[0] < 0.1f &&
		v[1] > -0.1f && v[1] < 0.1f &&
		v[2] > -0.1f && v[2] < 0.1f);
}

static void STEFX_PmoveSetDefaultBounds(void)
{
	...
	pm->maxs[2] = (pm->gent && pm->gent->client && pm->gent->client->standheight > 0) ?
		pm->gent->client->standheight : DEFAULT_MAXS_2;
}

static void STEFX_PmoveRepairState(const char *where, const float *fallbackOrigin, qboolean repairBounds)
{
	vec3_t fallback;
	qboolean badOrigin;
	qboolean badVelocity;
	qboolean badBounds;
	...
	VectorClear(fallback);
	if (fallbackOrigin && !STEFX_PmoveVec3Bad(fallbackOrigin))
	{
		VectorCopy(fallbackOrigin, fallback);
	}
	else if (pm->gent && !STEFX_PmoveVec3Bad(pm->gent->currentOrigin))
	{
		VectorCopy(pm->gent->currentOrigin, fallback);
	}

	badOrigin = STEFX_PmoveVec3Bad(pm->ps->origin);
	badVelocity = STEFX_PmoveVec3Bad(pm->ps->velocity);
	badBounds = (qboolean)(repairBounds && (STEFX_PmoveVec3Bad(pm->mins) || STEFX_PmoveVec3Bad(pm->maxs)));

	if (!badOrigin && !badVelocity && !badBounds)
	{
		return;
	}
...
{
	if (!STEFX_PmoveVec3Bad(pml.groundTrace.plane.normal) &&
		!STEFX_PmoveNearZero3D(pml.groundTrace.plane.normal))
	{
		return;
	}
...
static void PM_Friction( void ) {
...
#ifdef _XBOX
	static int s_stefxFrictionGuardLogCount = 0;
	STEFX_PmoveRepairState("friction-entry", pml.previous_origin, qfalse);
#endif
	...
	newspeed /= speed;
#ifdef _XBOX
	if (STEFX_PmoveFloatBad(newspeed))
	{
		if (s_stefxFrictionGuardLogCount < 16)
		{
...
#ifdef _XBOX
	static int	s_stefxAccelGuardLogCount = 0;

	if (STEFX_PmoveVec3Bad(wishdir) || STEFX_PmoveFloatBad(wishspeed) ||
		STEFX_PmoveFloatBad(accel) || STEFX_PmoveVec3Bad(pm->ps->velocity))
	{
...
#ifdef _XBOX
	if (STEFX_PmoveFloatBad(currentspeed) || STEFX_PmoveFloatBad(addspeed))
	{
		if (s_stefxAccelGuardLogCount < 16)
		{
...
#ifdef _XBOX
	if (STEFX_PmoveFloatBad(accelspeed))
	{
		if (s_stefxAccelGuardLogCount < 16)
		{
...
static void PM_AirMove( void ) {
	...
	if (!fmove && !smove && STEFX_PmoveNearZero2D(pm->ps->velocity))
	{
		...
		if (s_stefxStationaryWalkSkipCount < 4)
		{
			XBLF("STEFX: PM_AirMove stationary skip ent=%d origin=(%g,%g,%g) vel=(%g,%g,%g) groundNormal=(%g,%g,%g)",
				(pm && pm->gent) ? pm->gent->s.number : -1,
				pm->ps->origin[0], pm->ps->origin[1], pm->ps->origin[2],
				pm->ps->velocity[0], pm->ps->velocity[1], pm->ps->velocity[2],
				pml.groundTrace.plane.normal[0], pml.groundTrace.plane.normal[1], pml.groundTrace.plane.normal[2]);
			s_stefxStationaryWalkSkipCount++;
		}
		return;
	}
#endif
...
static void PM_WalkMove( void ) {
...
#ifdef _XBOX
	STEFX_PmoveRepairGroundNormal("walk-entry");
	STEFX_PmoveRepairState("walk-entry", pml.previous_origin, qfalse);
#endif
	...
	if ( PM_CheckJump () ) {
		// jumped away
#ifdef _XBOX
		STEFX_PmoveRepairState("walk-after-checkjump", pml.previous_origin, qfalse);
#endif
		if ( pm->waterlevel > 1 ) {
			PM_WaterMove();
		} else {
	...
	PM_Friction ();
#ifdef _XBOX
	STEFX_PmoveRepairState("walk-after-friction", pml.previous_origin, qfalse);
#endif
	...
	if (!fmove && !smove && STEFX_PmoveNearZero2D(pm->ps->velocity))
	{
		...
		if (s_stefxStationaryGroundSkipCount < 4)
		{
			XBLF("STEFX: PM_WalkMove stationary skip ent=%d origin=(%g,%g,%g) vel=(%g,%g,%g) groundNormal=(%g,%g,%g)",
				(pm && pm->gent) ? pm->gent->s.number : -1,
				pm->ps->origin[0], pm->ps->origin[1], pm->ps->origin[2],
				pm->ps->velocity[0], pm->ps->velocity[1], pm->ps->velocity[2],
				pml.groundTrace.plane.normal[0], pml.groundTrace.plane.normal[1], pml.groundTrace.plane.normal[2]);
			s_stefxStationaryGroundSkipCount++;
		}
		VectorClear(pm->ps->velocity);
		return;
	}
...
#ifdef _XBOX
	STEFX_PmoveRepairState("walk-after-accelerate", pml.previous_origin, qfalse);
#endif
...
//		pm->ps->velocity[2] = 0;
	...
	PM_ClipVelocity (pm->ps->velocity, pml.groundTrace.plane.normal, 
		pm->ps->velocity, OVERCLIP );
#ifdef _XBOX
	STEFX_PmoveRepairState("walk-after-clip", pml.previous_origin, qfalse);
#endif
...
void Pmove( pmove_t *pmove )
{
	...
	if (stefxPmoveVerbose) XBLF("STEFX: Pmove enter pmove=%08x ps=%08x gent=%08x weapon=%d buttons=0x%x serverTime=%d commandTime=%d",
		...
		(pmove && pmove->ps) ? pmove->ps->weapon : -1,
		pmove ? pmove->cmd.buttons : 0,
		pmove ? pmove->cmd.serverTime : -1,
		(pmove && pmove->ps) ? pmove->ps->commandTime : -1);
	STEFX_PmoveRepairState("entry", (pm && pm->gent) ? pm->gent->currentOrigin : NULL, qfalse);
#endif
	...
	if (stefxPmoveVerbose) XBLF("STEFX: Pmove weapon data weapon=%d ammoIndex=%d energy=%d altEnergy=%d ammo0=%d ammo1=%d ammo2=%d ammo3=%d",
		...
		pm->ps->ammo[0], pm->ps->ammo[1], pm->ps->ammo[2], pm->ps->ammo[3]);
#endif
	...
	else 
	{
		...
		{
#ifdef _XBOX
			static qboolean s_loggedSkipFFStopAllFX = qfalse;
			if (!s_loggedSkipFFStopAllFX)
			{
				XBLog_Write("STEFX: Pmove skipped game-side cgi_FF_StopAllFX on Xbox");
				s_loggedSkipFFStopAllFX = qtrue;
			}
#else
			cgi_FF_StopAllFX();
#endif
...
#ifdef _XBOX
	XBLF("STEFX: Pmove before PM_SetSpecialMoveValues msec=%d", pml.msec);
#endif
	PM_SetSpecialMoveValues();
#ifdef _XBOX
	XBLF("STEFX: Pmove after PM_SetSpecialMoveValues");
#endif
...
#ifdef _XBOX
	XBLF("STEFX: Pmove before PM_UpdateViewAngles");
#endif
	PM_UpdateViewAngles( pm->ps, &pm->cmd, pm->gent);
#ifdef _XBOX
	XBLF("STEFX: Pmove after PM_UpdateViewAngles");
#endif
...
#ifdef _XBOX
	XBLF("STEFX: Pmove before PM_SetWaterLevel");
#endif
	PM_SetWaterLevel();
#ifdef _XBOX
...
#ifdef _XBOX
	STEFX_PmoveRepairState("after-move", pml.previous_origin, qtrue);
#endif
	...
	PM_SetWaterLevel();
#ifdef _XBOX
	STEFX_PmoveRepairGroundNormal("after-final-groundtrace");
	STEFX_PmoveRepairState("after-final-groundtrace", pml.previous_origin, qtrue);
#endif
...
#ifdef _XBOX
	STEFX_PmoveRepairState("exit", pml.previous_origin, qtrue);
#endif
