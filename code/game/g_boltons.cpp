...
static qboolean STEFX_BoltOnIsHelmetName( const char *boltOnName )
{
...
#if defined(_XBOX) && defined(STEFX_ELITE_FORCE_SP)
	qboolean traceHelmet = STEFX_BoltOnIsHelmetName( boltOnName );
	if ( traceHelmet )
	{
...
byte G_BoltOnNumberForName( gentity_t *ent, const char *boltOnName )
{
	...
	for ( int i = 0; i < MAX_BOLT_ONS; i++ )
	{
		int knownIndex = ent->client->renderInfo.boltOns[i].index;
		if ( knownIndex < 0 || knownIndex >= numBoltOns )
		{
			continue;
		}

		if ( !Q_stricmp( boltOnName, knownBoltOns[knownIndex].name ) )
		{//Found it, return it
			return i;
		}
	}
