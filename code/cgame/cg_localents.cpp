...
void CG_AddFragment( localEntity_t *le ) 
{
	...
	#ifdef _XBOX
	{
		...
		{
			XBLF( "STEFX: EF CG_AddFragment trajectory le=%p leType=%d posType=%d posTime=%d posDuration=%d aposType=%d aposTime=%d aposDuration=%d time=%d origin=(%g,%g,%g)",
				le,
				le->leType,
				(int)le->pos.trType,
				le->pos.trTime,
...
void CG_AddLocalEntities( void ) 
{
	...
	for ( ; le != &cg_activeLocalEntities ; le = next ) {
		...
		{
			XBLF( "STEFX: EF CG_AddLocalEntities le=%p leType=%d start=%d end=%d posType=%d posTime=%d time=%d",
				le,
				le->leType,
				le->startTime,
				le->endTime,
