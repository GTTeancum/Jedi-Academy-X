...
private:
	...
	HRESULT InitD3DX()
	{
		HRESULT hr;
#ifdef _XBOX
		Con_Printf("FAKEGL: InitD3DX entry\n");
#endif
		m_pD3D = Direct3DCreate8(D3D_SDK_VERSION);
#ifdef _XBOX
		Con_Printf("FAKEGL: Direct3DCreate8 -> m_pD3D=%p\n", (void*)m_pD3D);
#endif
	...
	/*
		...
		*/
...
#ifdef _XBOX
		Con_Printf("FAKEGL: CreateDevice calling (w=%u h=%u fmt=%d flags=HW_VP|PUREDEVICE)\n",
			(unsigned)params.BackBufferWidth, (unsigned)params.BackBufferHeight,
			(int)params.BackBufferFormat);
#endif
...
#ifdef _XBOX
		Con_Printf("FAKEGL: CreateDevice -> hr=0x%08x m_pD3DDev=%p\n",
			(unsigned)hr, (void*)m_pD3DDev);
#endif
...
#ifdef _XBOX
			Con_Printf("FAKEGL: GetBackBuffer calling\n");
#endif
			m_pD3DDev->GetBackBuffer( 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer );
#ifdef _XBOX
			Con_Printf("FAKEGL: GetBackBuffer -> pBackBuffer=%p\n", (void*)pBackBuffer);
#endif
		...
		m_bD3DXReady = TRUE;
#ifdef _XBOX
		Con_Printf("FAKEGL: InitD3DX OK m_bD3DXReady=TRUE\n");
#endif
...
#ifdef _XBOX
		Con_Printf("FAKEGL: glBegin mode=0x%04x needBeginScene=%d m_pD3DDev=%p\n",
			(unsigned)mode, m_needBeginScene ? 1 : 0, (void*)m_pD3DDev);
#endif
		if ( m_needBeginScene ){
			m_needBeginScene = false;
#ifdef _XBOX
			Con_Printf("FAKEGL: glBegin calling BeginScene on m_pD3DDev=%p\n", (void*)m_pD3DDev);
#endif
			HRESULT hr = m_pD3DDev->BeginScene();
#ifdef _XBOX
			Con_Printf("FAKEGL: BeginScene -> hr=0x%08x\n", (unsigned)hr);
#endif
			if ( FAILED(hr) ) {
				InterpretError(hr);
			}
