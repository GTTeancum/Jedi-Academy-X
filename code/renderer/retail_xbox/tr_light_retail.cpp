...
{ //rwwRMG - modified args
	...
	STEFX_RETAIL_SCOPE R_TransformDlights( tr.refdef.num_dlights, tr.refdef.dlights, &tr.or );
...
void R_SetupEntityLighting( const trRefdef_t *refdef, trRefEntity_t *ent ) {
	...
	if ( !(refdef->rdflags & RDF_NOWORLDMODEL ) 
		...
		STEFX_RETAIL_SCOPE R_SetupEntityLightingGrid( ent );
	} else {
	...
	// Elite Force owns the render-flag contract used for entity minimum light.
...
qboolean RE_GetLighting( const vec3_t origin, vec3_t ambientLight, vec3_t directedLight, vec3_t lightDir )
{
	...
	STEFX_RETAIL_SCOPE R_SetupEntityLightingGrid( &tr_ent );
