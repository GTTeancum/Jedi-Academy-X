...
static qboolean	R_CullSurface( surfaceType_t *surface, shader_t *shader ) {
	...
	#if 0 // Jedi Academy automap screenshot roof culling is not an Elite Force renderer feature.
...
{
#ifdef _XBOX
	int stefxProbeSurfaceIndex = -1;
	unsigned int stefxProbeSurfaceMask = 0;
	volatile unsigned int *stefxProbeSeenWord = NULL;
	if (s_stefxHMPvsProbeWorldWalk && tr.world && tr.world->surfaces &&
	...
	*/
...
#endif
	{
		...
		{
			g_SPXBHMPvsProbe[
				STEFX_HM_PVS_PROBE_ADDED_BASE + (stefxProbeSurfaceIndex >> 5)] |= stefxProbeSurfaceMask;
			g_SPXBHMPvsProbe[55]++;
		}
#endif
	}
...
void R_AddBrushModelSurfaces ( trRefEntity_t *ent ) {
	...
	{ //rwwRMG - added
		STEFX_RETAIL_SCOPE R_SetupEntityLighting(&tr.refdef, ent);
	}
...
//	{	// don't dlight bmodels on rmg, as multiple copies of the same instance will light up
		STEFX_RETAIL_SCOPE R_DlightBmodel( bmodel, false );
//	}
//	else
//	{
...
=============================================================
*/
#if 0 // Jedi Academy wireframe automap is outside the Elite Force renderer contract.
	...
	do
	{
...
#endif
		{
			...
			if ( planeBits & 1 ) {
				...
				if (r == 2) {
#ifdef _XBOX
					if (s_stefxHMPvsProbeWorldWalk) g_SPXBHMPvsProbe[49]++;
#endif
					return;						// culled
				}
				if ( r == 1 ) {
...
void R_MarkLeaves (mleaf_s *leafOverride) {
	...
	int		cluster;
	int		stefxIgnorePvs;
	int		stefxIgnoreAreaMask;
	unsigned int stefxProbeSerial;
	unsigned int stefxProbeHash;
	unsigned int stefxProbeBits;
	unsigned int stefxProbeMarked;
	unsigned int stefxProbePvsRejected;
	unsigned int stefxProbeAreaRejected;
	unsigned int stefxProbeInvalid;
	unsigned int stefxProbeWord;
	int stefxProbeByte;
	int stefxProbeBit;
	int stefxProbeWordIndex;
	qboolean stefxProbePrimary;
	static int stefxLastIgnorePvs = -1;
	static int stefxLastIgnoreAreaMask = -1;
	...
	stefxIgnorePvs = Cvar_VariableIntegerValue("stefx_hm_smoke_ignore_pvs");
	stefxIgnoreAreaMask = Cvar_VariableIntegerValue("stefx_hm_smoke_ignore_areamask");
	stefxProbePrimary = !tr.viewParms.isPortal;
	...
	if ( tr.viewCluster == cluster && !tr.refdef.areamaskModified &&
		 stefxIgnorePvs == stefxLastIgnorePvs &&
		 stefxIgnoreAreaMask == stefxLastIgnoreAreaMask ) {
		return;
	}

	tr.visCount++;
	tr.viewCluster = cluster;
	stefxLastIgnorePvs = stefxIgnorePvs;
	stefxLastIgnoreAreaMask = stefxIgnoreAreaMask;
	...
	stefxProbeSerial = g_SPXBHMPvsProbe[0];
	if (stefxProbePrimary) {
		stefxProbeSerial++;
		if ( !(stefxProbeSerial & 1) ) {
			stefxProbeSerial++;
		}
		g_SPXBHMPvsProbe[0] = stefxProbeSerial;
		g_SPXBHMPvsProbe[1] = (unsigned int)tr.viewCluster;
		g_SPXBHMPvsProbe[2] = (unsigned int)tr.world->numClusters;
		g_SPXBHMPvsProbe[3] = (unsigned int)tr.world->clusterBytes;
		g_SPXBHMPvsProbe[6] = (unsigned int)vis;
		g_SPXBHMPvsProbe[7] = (unsigned int)tr.world->numleafs;
		g_SPXBHMPvsProbe[12] = (unsigned int)stefxIgnorePvs;
		g_SPXBHMPvsProbe[13] = (unsigned int)stefxIgnoreAreaMask;
		g_SPXBHMPvsProbe[14] = (unsigned int)leaf->cluster;
		g_SPXBHMPvsProbe[15] = (unsigned int)leaf->area;
	}

	stefxProbeHash = 2166136261u;
	stefxProbeBits = 0;
	for ( stefxProbeByte = 0; stefxProbeByte < tr.world->clusterBytes; stefxProbeByte++ ) {
		unsigned int stefxProbeValue = vis[stefxProbeByte];
		stefxProbeHash = (stefxProbeHash ^ stefxProbeValue) * 16777619u;
		for ( stefxProbeBit = 0; stefxProbeBit < 8; stefxProbeBit++ ) {
			stefxProbeBits += (stefxProbeValue >> stefxProbeBit) & 1u;
		}
	}
	if (stefxProbePrimary) {
		g_SPXBHMPvsProbe[4] = stefxProbeHash;
		g_SPXBHMPvsProbe[5] = stefxProbeBits;
	}
	for ( stefxProbeWordIndex = 0; stefxProbePrimary && stefxProbeWordIndex < 24; stefxProbeWordIndex++ ) {
		stefxProbeWord = 0;
		for ( stefxProbeByte = 0; stefxProbeByte < 4; stefxProbeByte++ ) {
			int stefxProbeOffset = stefxProbeWordIndex * 4 + stefxProbeByte;
			if ( stefxProbeOffset < tr.world->clusterBytes ) {
				stefxProbeWord |= ((unsigned int)vis[stefxProbeOffset]) << (stefxProbeByte * 8);
			}
		}
		g_SPXBHMPvsProbe[16 + stefxProbeWordIndex] = stefxProbeWord;
	}

	stefxProbeMarked = 0;
	stefxProbePvsRejected = 0;
	stefxProbeAreaRejected = 0;
	stefxProbeInvalid = 0;
	...
	for (i=0,leaf=tr.world->leafs ; i<tr.world->numleafs ; i++, leaf++) {
		...
		// check general pvs
		if ( !stefxIgnorePvs && !(vis[cluster>>3] & (1<<(cluster&7))) ) {
			stefxProbePvsRejected++;
			continue;
		}
	...
	g_SPXBHMPvsProbe[40] = (unsigned int)tr.world->nodes[0].visframe;
	g_SPXBHMPvsProbe[41] = (unsigned int)tr.visCount;
	g_SPXBHMPvsProbe[42] = 0;
	for (i = 0; i < tr.world->numnodes; i++) {
		if (tr.world->nodes[i].visframe == tr.visCount) {
			g_SPXBHMPvsProbe[42]++;
		}
	}
	g_SPXBHMPvsProbe[43] = 0;
	g_SPXBHMPvsProbe[44] = 0;
	g_SPXBHMPvsProbe[45] = 0;
	g_SPXBHMPvsProbe[46] = 0;
	for (i = 0, leaf = tr.world->leafs; i < tr.world->numleafs; i++, leaf++) {
		mnode_t *stefxProbeParent;
		mnode_t *stefxProbeChild;
		int stefxProbeDepth;
		if (leaf->visframe == tr.visCount) {
			g_SPXBHMPvsProbe[43]++;
		}
		if (leaf->cluster < 0 || leaf->cluster >= tr.world->numClusters ||
			(!stefxIgnorePvs && !(vis[leaf->cluster >> 3] & (1 << (leaf->cluster & 7))))) {
			continue;
		}
		if (leaf->visframe != tr.visCount) {
			g_SPXBHMPvsProbe[44]++;
		}
		stefxProbeChild = (mnode_t *)leaf;
		stefxProbeParent = stefxProbeChild->parent;
		stefxProbeDepth = 0;
		while (stefxProbeParent && stefxProbeDepth <= tr.world->numnodes) {
			if (stefxProbeParent < tr.world->nodes ||
				stefxProbeParent >= tr.world->nodes + tr.world->numnodes) {
				g_SPXBHMPvsProbe[45]++;
				break;
			}
			if (stefxProbeParent->children[0] != stefxProbeChild &&
				stefxProbeParent->children[1] != stefxProbeChild) {
				g_SPXBHMPvsProbe[45]++;
				break;
			}
			stefxProbeChild = stefxProbeParent;
			stefxProbeParent = stefxProbeParent->parent;
			stefxProbeDepth++;
		}
		if (stefxProbeChild != tr.world->nodes) {
			g_SPXBHMPvsProbe[46]++;
		}
	}
	g_SPXBHMPvsProbe[8] = stefxProbeMarked;
	g_SPXBHMPvsProbe[9] = stefxProbePvsRejected;
	g_SPXBHMPvsProbe[10] = stefxProbeAreaRejected;
	g_SPXBHMPvsProbe[11] = stefxProbeInvalid;
	g_SPXBHMPvsProbe[0] = stefxProbeSerial + 1;
}
#else // _XBOX
...
void R_AddWorldSurfaces (void) {
	...
	stefxProbePrimary = !tr.viewParms.isPortal;
	if (stefxProbePrimary) {
		for (stefxProbeWordIndex = 47; stefxProbeWordIndex < 296; stefxProbeWordIndex++) {
			g_SPXBHMPvsProbe[stefxProbeWordIndex] = 0;
		}
	}
	...
	if (stefxProbePrimary) {
		g_SPXBHMPvsProbe[57] = (unsigned int)(tr.refdef.numDrawSurfs - stefxProbeFirstDrawSurf);
		g_SPXBHMPvsProbe[58] = *(unsigned int *)&tr.viewParms.visBounds[0][0];
		g_SPXBHMPvsProbe[59] = *(unsigned int *)&tr.viewParms.visBounds[0][1];
		g_SPXBHMPvsProbe[60] = *(unsigned int *)&tr.viewParms.visBounds[0][2];
		g_SPXBHMPvsProbe[61] = *(unsigned int *)&tr.viewParms.visBounds[1][0];
		g_SPXBHMPvsProbe[62] = *(unsigned int *)&tr.viewParms.visBounds[1][1];
		g_SPXBHMPvsProbe[63] = *(unsigned int *)&tr.viewParms.visBounds[1][2];
		g_SPXBHMPvsProbe[64] = (unsigned int)tr.world->nodes[0].visframe;
		g_SPXBHMPvsProbe[65] = (unsigned int)tr.visCount;
	}
}
