...
static void DoLine_Oriented( const vec3_t start, const vec3_t end, const vec3_t up, float spanWidth )
{
	...
	tess.texCoords[tess.numVertexes][0][0] = 0;
	tess.texCoords[tess.numVertexes][0][1] = backEnd.currentEntity->e.stefxData.line.stscale;
	tess.vertexColors[tess.numVertexes][0] = backEnd.currentEntity->e.shaderRGBA[0];
	tess.vertexColors[tess.numVertexes][1] = backEnd.currentEntity->e.shaderRGBA[1];
	tess.vertexColors[tess.numVertexes][2] = backEnd.currentEntity->e.shaderRGBA[2];
	tess.vertexColors[tess.numVertexes][3] = backEnd.currentEntity->e.shaderRGBA[3];// * 0.25;
...
static void RB_SurfaceOrientedLine( void ) 
{
	...
	DoLine_Oriented( start, end, right, e->stefxData.line.width*0.5 );
}
#endif
...
static bool RB_TestZFlare( vec3_t point) {
	...
	STEFX_RETAIL_SCOPE R_TransformModelToClip( point, backEnd.ori.modelMatrix, 
	...
	STEFX_RETAIL_SCOPE R_TransformClipToWindow( clip, &backEnd.viewParms, normalized, window );
...
void RB_SurfaceDisplayList( void *surface ) {
...
#ifdef STEFX_ELITE_FORCE_SP
	(void(*)(void*))RB_SurfaceAnim			// SF_MDR is an Elite Force extension.
#endif
};
