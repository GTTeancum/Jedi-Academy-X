...
// The shipping MP renderer was built against JA's qcommon/game contracts. Keep
// its shader construction intact while translating those narrow contracts to
// the Elite Force engine that owns this renderer.
...
qhandle_t RE_RegisterShaderLightMap( const char *name, const short *lightmapIndex,
	const byte *styles );
...
// Collision and rendering retain separate parsed views of the shared EF
// shader assets. This buffer belongs to the renderer and follows its hunk
// registration lifetime.
static char *shaderText;
...
infoParm_t	infoParms[] = {
	...
	{"botclip",		~CONTENTS_SOLID,	0,				CONTENTS_BOTCLIP },
	{"trigger",		~CONTENTS_SOLID,	0,				CONTENTS_TRIGGER },
	{"nodrop",		~CONTENTS_SOLID,	0,				CONTENTS_NODROP },
	{"terrain",		-1,					0,				0 },
	{"ladder",		~CONTENTS_SOLID,	0,				CONTENTS_LADDER },
	{"abseil",		-1,					0,				0 },
	{"outside",		-1,					0,				0 },
	{"inside",		-1,					0,				0 },
...
#ifdef _XBOX
static qboolean R_XboxLoadShaderManifest( const char *shaderDir, char **manifestFiles,
	int maxFiles, char **manifestTextOut, int *numShaderOut )
{
...
void ScanAndLoadShaderFiles( const char *path, bool doHash )
{
	...
	// The retail parser owns the resulting text and hash table. Elite Force's
	// packaged shader corpus lives under scripts/, with shaders/ retained for
	...
	if( !shaderText )
	{
		...
		int dirIndex;
		long sum = 0;
		const char *shaderDirs[] = { "scripts", "shaders" };
#ifdef _XBOX
		char *manifestFiles[MAX_SHADER_FILES];
		char *manifestText;
		qboolean manifestActive;
		(void)path;
		g_SPXBShaderScanMagic = 0x52534631; /* 'RSF1' */
		...
		for ( dirIndex = 0; dirIndex < 2 && totalShaders < MAX_SHADER_FILES; ++dirIndex ) {
			shaderFiles = NULL;
			numShaders = 0;
#ifdef _XBOX
			manifestText = NULL;
...
#ifdef _XBOX
			if ( ( !shaderFiles || !numShaders ) && dirIndex == 0 ) {
				if ( shaderFiles ) {
			...
			for ( i = 0; i < numShaders; ++i ) {
				...
				bufferSizes[totalShaders] = COM_Compress( buffers[totalShaders] );
				sum += bufferSizes[totalShaders];
				++totalShaders;
#ifdef _XBOX
		...
		shaderText = (char *) Z_Malloc( sum + totalShaders * 2, TAG_SHADERTEXT, qfalse, 4 );
		sum = 0;
		shaderText[0] = 0;
		for ( i = totalShaders - 1; i >= 0 ; --i ) {
			strcat( shaderText + sum, "\n" );
			strcat( shaderText + sum, buffers[i] );
			FS_FreeFile( (void*) buffers[i] );
			sum += bufferSizes[i] + 1;
		}
#ifdef _XBOX
		g_SPXBShaderScanMagic = 0x52534632; /* 'RSF2' */
#endif
	}
	...
	for (i = 0; i < MAX_SHADERTEXT_HASH; ++i) {
		shaderTextHashTable[i][shaderTextHashTableSizes[i]] = NULL;
	}

#ifdef _XBOX
	g_SPXBShaderScanMagic = 0x52534633; /* 'RSF3' */
#endif
