...
static int s_stefxModelRegisterLogBudget = 96;

static void STEFX_LogModelRegisterPhase(const char *phase, const char *modelName, const char *skinName, const char *filename, int handle)
{
	if (s_stefxModelRegisterLogBudget > 0) {
		CG_Printf("STEFX_HM_MODEL: phase=%s model=%s skin=%s file=%s handle=%d mem=%d\n",
			phase ? phase : "?",
			modelName ? modelName : "",
			skinName ? skinName : "",
			filename ? filename : "",
...
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("lower-mdr-enter", modelName, skinName, filename, 0);
#endif
	ci->legsModel = trap_R_RegisterModel( filename );
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("lower-mdr-exit", modelName, skinName, filename, ci->legsModel);
#endif
	if ( !ci->legsModel ) {
		Com_sprintf( filename, sizeof( filename ), "models/players2/%s/lower.md3", modelName );
#if defined(STEFX_SP_HOSTED_MP)
		STEFX_LogModelRegisterPhase("lower-md3-enter", modelName, skinName, filename, 0);
#endif
		ci->legsModel = trap_R_RegisterModel( filename );
#if defined(STEFX_SP_HOSTED_MP)
		STEFX_LogModelRegisterPhase("lower-md3-exit", modelName, skinName, filename, ci->legsModel);
#endif
		if ( !ci->legsModel ) {
...
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("upper-mdr-enter", modelName, skinName, filename, 0);
#endif
	ci->torsoModel = trap_R_RegisterModel( filename );
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("upper-mdr-exit", modelName, skinName, filename, ci->torsoModel);
#endif
	if ( !ci->torsoModel ) {
		Com_sprintf( filename, sizeof( filename ), "models/players2/%s/upper.md3", modelName );
#if defined(STEFX_SP_HOSTED_MP)
		STEFX_LogModelRegisterPhase("upper-md3-enter", modelName, skinName, filename, 0);
#endif
		ci->torsoModel = trap_R_RegisterModel( filename );
#if defined(STEFX_SP_HOSTED_MP)
		STEFX_LogModelRegisterPhase("upper-md3-exit", modelName, skinName, filename, ci->torsoModel);
#endif
		if ( !ci->torsoModel ) {
...
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("head-enter", modelName, skinName, filename, 0);
#endif
	ci->headModel = trap_R_RegisterModel( filename );
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("head-exit", modelName, skinName, filename, ci->headModel);
#endif
	if ( !ci->headModel ) {
		Com_Printf( "Failed to load model file %s\n", filename );
		return qfalse;
...
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("skin-enter", modelName, skinName, "", 0);
#endif
...
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("skin-exit", modelName, skinName, "", 1);
#endif

	// load the animations
	Com_sprintf( filename, sizeof( filename ), "models/players2/%s/animation.cfg", modelName );
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("animation-enter", modelName, skinName, filename, 0);
#endif
...
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("animation-exit", modelName, skinName, filename, 1);
#endif

	Com_sprintf( filename, sizeof( filename ), "models/players2/%s/icon_%s.jpg", modelName, skinName );
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("icon-enter", modelName, skinName, filename, 0);
#endif
	ci->modelIcon = trap_R_RegisterShaderNoMip( filename );
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_LogModelRegisterPhase("icon-exit", modelName, skinName, filename, ci->modelIcon);
#endif
	if ( !ci->modelIcon ) {
