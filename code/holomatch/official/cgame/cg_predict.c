	...
	if (!(cent->currentState.eFlags & (EF_ITEMPLACEHOLDER | EF_NODRAW)))
	{
		...
		if ( item->giType == IT_WEAPON )
		{
			...
			if ( item->giTag >= 0 && item->giTag < WP_NUM_WEAPONS ) {
				...
				if ( stefxOriginalFlags & EF_DEAD ) {
					quantity = item->quantity;
				} else if ( cg.predictedPlayerState.ammo[item->giTag] < quantity * 0.5f ) {
					quantity = quantity - cg.predictedPlayerState.ammo[item->giTag];
				} else {
					quantity = quantity * 0.5f;
				}
				cg.predictedPlayerState.ammo[item->giTag] += quantity;
				if ( cg.predictedPlayerState.ammo[item->giTag] > Max_Ammo[item->giTag] ) {
					cg.predictedPlayerState.ammo[item->giTag] = Max_Ammo[item->giTag];
				}
			}
#else
		...
		if ( s_stefxPredictPickupLogs < 96 ) {
			...
			XBLog_WriteCriticalf("STEFX_HM_AMMO: predict-pickup client=%d item=%d type=%d tag=%d flags=0x%x originalFlags=0x%x ammo=%d->%d weapons=0x%x time=%d",
				...
				stefxAmmoBefore,
				stefxAmmoAfter,
				cg.predictedPlayerState.stats[STAT_WEAPONS],
				cg.time);
			++s_stefxPredictPickupLogs;
		}
#endif
	}
	...
	for ( i = 0 ; i < cg_numTriggerEntities ; i++ ) {
...
#if defined(STEFX_SP_HOSTED_MP) && defined(STEFX_HM_GAMEPLAY_DIAGNOSTICS)
		{
			...
			if ( stefxTriggerPredictionBudget > 0 ) {
				XBLog_WriteCriticalf("STEFX_HM_TRIGGER_PREDICT: type=%d ent=%d model=%d origin=(%g,%g,%g) angles=(%g,%g,%g) player=(%g,%g,%g) time=%d",
					ent->eType, ent->number, ent->modelindex,
					origin[0], origin[1], origin[2],
					angles[0], angles[1], angles[2],
					cg.predictedPlayerState.origin[0], cg.predictedPlayerState.origin[1], cg.predictedPlayerState.origin[2],
					cg.time);
				--stefxTriggerPredictionBudget;
			}
		}
#endif
	...
	if ( cg.thisFrameTeleport ) {
		...
		if (stefxTeleportResetBudget > 0) {
			XBLog_WriteCriticalf("STEFX_HM_PREDICTION: hosted teleport reset time=%d snapTime=%d commandTime=%d origin=(%g,%g,%g) flags=0x%x",
				cg.time, cg.snap->serverTime, cg.predictedPlayerState.commandTime,
				cg.predictedPlayerState.origin[0], cg.predictedPlayerState.origin[1], cg.predictedPlayerState.origin[2],
				...
				if ( len > 0.1 ) {
					...
					if (stefxPredictionMissBudget > 0) {
						XBLog_WriteCriticalf("STEFX_HM_PREDICTION: miss len=%g delta=(%g,%g,%g) predicted=(%g,%g,%g) server=(%g,%g,%g) cmdTime=%d ground=%d pmFlags=0x%x",
							...
							oldPlayerState.origin[0], oldPlayerState.origin[1], oldPlayerState.origin[2],
							adjusted[0], adjusted[1], adjusted[2],
							cg.predictedPlayerState.commandTime,
							cg.predictedPlayerState.groundEntityNum,
							cg.predictedPlayerState.pm_flags);
						--stefxPredictionMissBudget;
					}
#endif
					if ( cg_showmiss.integer ) {
