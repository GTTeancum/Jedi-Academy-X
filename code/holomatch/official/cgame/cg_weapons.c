...
*/
...
#if defined(STEFX_SP_HOSTED_MP)
	{
		...
		if ( stefxClientNum >= 0 && stefxClientNum < 3 &&
			s_stefxBeamLogBudget[stefxClientNum] > 0 )
		{
			XBLog_WriteCriticalf("STEFX_WEAPON: CG_LightningBolt client=%d time=%d weapon=%d eFlags=0x%x start=(%g,%g,%g) end=(%g,%g,%g) fraction=%g hit=%d",
				stefxClientNum,
				cg.time,
				...
				trace.fraction,
				trace.entityNum);
			--s_stefxBeamLogBudget[stefxClientNum];
		}
	}
#endif
...
void CG_NextWeapon_f( void ) {
	...
	if ( s_stefxWeaponSelectLogBudget > 0 ) {
		XBLog_WriteCriticalf("STEFX_HM_AMMO: cgame-weapnext time=%d original=%d selected=%d weapons=0x%x ammoSelected=%d",
			...
			original,
			cg.weaponSelect,
			cg.snap->ps.stats[STAT_WEAPONS],
			(cg.weaponSelect >= 0 && cg.weaponSelect < WP_NUM_WEAPONS) ? cg.snap->ps.ammo[cg.weaponSelect] : -1);
		--s_stefxWeaponSelectLogBudget;
	}
#endif
}
	...
	if ( ! ( cg.snap->ps.stats[STAT_WEAPONS] & ( 1 << num ) ) ) {
		...
		if ( s_stefxWeaponSelectLogBudget > 0 ) {
			XBLog_WriteCriticalf("STEFX_HM_AMMO: cgame-weapon reject time=%d requested=%d mapped=%d weapons=0x%x ammoMapped=%d",
				...
				num,
				cg.snap->ps.stats[STAT_WEAPONS],
				(num >= 0 && num < WP_NUM_WEAPONS) ? cg.snap->ps.ammo[num] : -1);
			--s_stefxWeaponSelectLogBudget;
		}
#endif
...
void CG_FireWeapon( centity_t *cent, qboolean alt_fire ) {
...
#if defined(STEFX_SP_HOSTED_MP)
	{
		...
		{
			XBLog_WriteCriticalf("STEFX_WEAPON: CG_FireWeapon client=%d time=%d alt=%d weapon=%d eFlags=0x%x lightning=%d",
				ent->number,
				cg.time,
				alt_fire ? 1 : 0,
				ent->weapon,
