...
void CG_CheckPlayerstateEvents( playerState_t *ps, playerState_t *ops ) {
	...
	for ( i = ps->eventSequence - MAX_PS_EVENTS ; i < ps->eventSequence ; i++ ) {
		...
		if ( i >= ops->eventSequence
			...
			{
				static int s_stefxMovementEventLogBudget = 256;
				if ( s_stefxMovementEventLogBudget > 0 )
				{
					XBLog_WriteCriticalf("STEFX_HM_EVENT_CLIENT: time=%d index=%d event=%d parm=%d psSeq=%d oldSeq=%d ring=(%d,%d,%d,%d)",
						cg.time,
						i,
						event,
						...
						ps->eventSequence,
						ops->eventSequence,
						ps->events[0], ps->events[1], ps->events[2], ps->events[3]);
					--s_stefxMovementEventLogBudget;
				}
			}
#endif
			CG_EntityEvent( cent, cent->lerpOrigin );
