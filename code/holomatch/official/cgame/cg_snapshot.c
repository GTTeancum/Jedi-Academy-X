...
static void CG_TransitionSnapshot( void ) {
	...
	if ( oldFrame ) {
		...
		if ( ( ps->eFlags ^ ops->eFlags ) & EF_TELEPORT_BIT ) {
			...
			XBLog_WriteCriticalf("STEFX_HM_TELEPORT: transition oldTime=%d newTime=%d oldFlags=0x%x newFlags=0x%x oldCmd=%d newCmd=%d oldOrigin=(%g,%g,%g) newOrigin=(%g,%g,%g)",
				oldFrame->serverTime, cg.snap->serverTime,
				ops->eFlags, ps->eFlags,
				ops->commandTime, ps->commandTime,
				ops->origin[0], ops->origin[1], ops->origin[2],
...
#if defined(STEFX_SP_HOSTED_MP)
		XBLog_WriteCriticalf("STEFX_HM_TELEPORT: next currentTime=%d nextTime=%d currentFlags=0x%x nextFlags=0x%x currentCmd=%d nextCmd=%d",
			cg.snap->serverTime, snap->serverTime,
			cg.snap->ps.eFlags, snap->ps.eFlags,
			cg.snap->ps.commandTime, snap->ps.commandTime);
#endif
