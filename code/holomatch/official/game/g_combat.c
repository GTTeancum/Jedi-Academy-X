...
*/	
	...
	#if defined(STEFX_SP_HOSTED_MP)
	XBLog_WriteCriticalf("STEFX_HM_MATCH death time=%d victim=%d victimName='%s' killer=%d killerName='%s' mod=%d damage=%d victimScore=%d killerScore=%d",
		level.time,
		self->s.number,
	...
	#if defined(STEFX_SP_HOSTED_MP)
	XBLog_WriteCriticalf("STEFX_HM_MATCH scored time=%d victim=%d victimScore=%d killer=%d killerScore=%d",
		level.time,
		self->s.number,
		self->client->ps.persistant[PERS_SCORE],
		(attacker && attacker->client) ? attacker->s.number : ENTITYNUM_WORLD,
		(attacker && attacker->client) ? attacker->client->ps.persistant[PERS_SCORE] : 0);
	#endif
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	gentity_t	*evEnt;
#if defined(STEFX_SP_HOSTED_MP)
	int			stefxHealthBefore = targ ? targ->health : 0;
#endif
	...
	if (take)
	{
		...
		if (attacker && attacker->client &&
			targ && targ->client && targ != attacker)
		{
			XBLog_WriteCriticalf("STEFX_HM_DAMAGE: attacker=%d target=%d mod=%d raw=%d take=%d armor=%d health=%d->%d",
				attacker->s.number,
				targ->s.number,
				mod,
				damage,
