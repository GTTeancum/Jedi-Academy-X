...
//	gi.trace ( &tr, muzzle, NULL, NULL, end, ent->s.number, MASK_SHOT);
...
#if defined(STEFX_SP_HOSTED_MP) && defined(STEFX_HM_GAMEPLAY_DIAGNOSTICS)
	{
		...
		if ( s_stefxPhaserTraceLogBudget > 0 && ent && ent->client &&
			ent->s.number >= 0 && ent->s.number < 3 )
		{
			XBLog_WriteCriticalf("STEFX_WEAPON: PhaserTrace client=%d time=%d alt=%d psWeapon=%d stateWeapon=%d muzzle=(%g,%g,%g) end=(%g,%g,%g) fraction=%g hit=%d startsolid=%d allsolid=%d",
				ent->s.number,
				level.time,
				alt_fire ? 1 : 0,
				...
				tr.startsolid ? 1 : 0,
				tr.allsolid ? 1 : 0);
			--s_stefxPhaserTraceLogBudget;
		}
		if ( s_stefxClosePhaserTraceLogBudget > 0 && ent && ent->client &&
			(ent->s.number == 1 || ent->s.number == 2) &&
			g_entities[0].inuse && g_entities[0].client )
		{
			vec3_t targetDelta;
			float targetDistance;
			VectorSubtract(g_entities[0].client->ps.origin, ent->client->ps.origin, targetDelta);
			targetDistance = VectorLength(targetDelta);
			if ( targetDistance <= 160.0f )
			{
				XBLog_WriteCriticalf("STEFX_HM_CLOSE_FIRE time=%d shooter=%d distance=%g ammo=%d shooterOrigin=(%g,%g,%g) targetHealth=%d targetOrigin=(%g,%g,%g) muzzle=(%g,%g,%g) hit=%d fraction=%g end=(%g,%g,%g)",
					level.time,
					ent->s.number,
					targetDistance,
					ent->client->ps.ammo[WP_PHASER],
					ent->client->ps.origin[0], ent->client->ps.origin[1], ent->client->ps.origin[2],
					g_entities[0].client->ps.stats[STAT_HEALTH],
					g_entities[0].client->ps.origin[0], g_entities[0].client->ps.origin[1], g_entities[0].client->ps.origin[2],
					muzzle[0], muzzle[1], muzzle[2],
					tr.entityNum,
					tr.fraction,
					tr.endpos[0], tr.endpos[1], tr.endpos[2]);
				--s_stefxClosePhaserTraceLogBudget;
			}
		}
	}
#endif
	...
	for (i = 0; i < NUM_PHASER_TRACES; i++)
	{
...
#if defined(STEFX_SP_HOSTED_MP) && defined(STEFX_HM_GAMEPLAY_DIAGNOSTICS)
		{
			...
			if ( s_stefxPhaserTargetLogBudget > 0 && ent && ent->client &&
				ent->s.number >= 0 && ent->s.number < 3 )
			{
				XBLog_WriteCriticalf("STEFX_WEAPON: PhaserTarget client=%d time=%d trace=%d hit=%d targetClient=%d fraction=%g inuse=%d takedamage=%d health=%d eType=%d",
					ent->s.number,
					level.time,
					i,
					trEnts[i],
					...
					trEntFraction[i],
					traceEnt->inuse ? 1 : 0,
					traceEnt->takedamage ? 1 : 0,
					traceEnt->health,
					traceEnt->s.eType);
				--s_stefxPhaserTargetLogBudget;
			}
		}
#endif
...
void FireWeapon( gentity_t *ent, qboolean alt_fire ) 
{
	...
	if ( ent && ent->s.number >= 0 && ent->s.number < 3 && ent->client )
	{
		...
		{
			XBLog_WriteCriticalf("STEFX_WEAPON: FireWeapon client=%d time=%d alt=%d psWeapon=%d stateWeapon=%d ammo=%d origin=(%g,%g,%g) view=(%g,%g,%g)",
				ent->s.number,
				level.time,
				alt_fire ? 1 : 0,
				ent->client->ps.weapon,
