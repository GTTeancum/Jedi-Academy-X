...
static int s_stefxTriggerTeleportLogBudget = 64;
#endif
...
#if defined(STEFX_SP_HOSTED_MP)
	if ( other->s.number == 0 && s_stefxTriggerTeleportLogBudget > 0 )
	{
		XBLog_WriteCriticalf("STEFX_HM_TELEPORT: touch levelTime=%d trigger=%d random=%d transportSpeed=%d from=(%g,%g,%g) dest=(%g,%g,%g) angles=(%g,%g,%g) velocity=(%g,%g,%g) pmFlags=0x%x pmTime=%d",
			level.time,
			self->s.number,
			(self->spawnflags & SF_RANDOM) != 0,
			self->health,
			other->client->ps.origin[0], other->client->ps.origin[1], other->client->ps.origin[2],
			destPoint[0], destPoint[1], destPoint[2],
			dest->s.angles[0], dest->s.angles[1], dest->s.angles[2],
			other->client->ps.velocity[0], other->client->ps.velocity[1], other->client->ps.velocity[2],
			other->client->ps.pm_flags,
			other->client->ps.pm_time);
		--s_stefxTriggerTeleportLogBudget;
	}
#endif
