...
static int s_stefxPmoveAmmoLogCount = 0;

static void STEFX_PmoveLogAmmo(const char *phase, int beforePhaser, int beforeRecharge)
{
	if (pm && pm->ps && pm->ps->clientNum >= 0 && pm->ps->clientNum < 3 &&
		s_stefxPmoveAmmoLogCount < 160)
	{
		const int weapon = pm->ps->weapon;
		const int weaponAmmo = (weapon >= 0 && weapon < WP_NUM_WEAPONS) ? pm->ps->ammo[weapon] : -9999;
		XBLog_WriteCriticalf("STEFX_HM_AMMO: pmove-%s client=%d cmdTime=%d buttons=0x%x weapon=%d weaponAmmo=%d phaser=%d->%d hypo=%d recharge=%d->%d weaponTime=%d flags=0x%x",
			phase ? phase : "?",
			pm->ps->clientNum,
			pm->cmd.serverTime,
			pm->cmd.buttons,
			...
			pm->ps->ammo[WP_VOYAGER_HYPO],
			beforeRecharge,
			pm->ps->rechargeTime,
			pm->ps->weaponTime,
			pm->ps->pm_flags);
		++s_stefxPmoveAmmoLogCount;
	}
}

static void STEFX_PmoveLogBounds(const char *phase)
{
	if (pm && pm->cmd.serverTime >= 1000 && pm->cmd.serverTime <= 1200 &&
		s_stefxPmoveBoundsLogCount < 32)
	{
		XBLog_WriteCriticalf("STEFX_HM_SP: pmove bounds phase=%s origin=(%g,%g,%g) mins=(%g,%g,%g) maxs=(%g,%g,%g) flags=0x%x",
			phase ? phase : "?",
			pm && pm->ps ? pm->ps->origin[0] : 0.0f,
			pm && pm->ps ? pm->ps->origin[1] : 0.0f,
			pm && pm->ps ? pm->ps->origin[2] : 0.0f,
			pm ? pm->mins[0] : 0.0f,
			pm ? pm->mins[1] : 0.0f,
			pm ? pm->mins[2] : 0.0f,
			pm ? pm->maxs[0] : 0.0f,
			pm ? pm->maxs[1] : 0.0f,
			pm ? pm->maxs[2] : 0.0f,
			pm && pm->ps ? pm->ps->pm_flags : 0);
		++s_stefxPmoveBoundsLogCount;
	}
}

static void STEFX_PmoveLogState(const char *phase)
{
	if (pm && pm->cmd.serverTime >= 1000 && pm->cmd.serverTime <= 1200 &&
		s_stefxPmoveStateLogCount < 48)
	{
		XBLog_WriteCriticalf("STEFX_HM_SP: pmove state phase=%s client=%d cmdTime=%d ps=%p pointcontents=%p origin=(%g,%g,%g) velocity=(%g,%g,%g) flags=0x%x water=%d type=0x%x",
			phase ? phase : "?",
			pm && pm->ps ? pm->ps->clientNum : -1,
			pm ? pm->cmd.serverTime : -1,
			pm ? pm->ps : NULL,
			pm ? pm->pointcontents : NULL,
			pm && pm->ps ? pm->ps->origin[0] : 0.0f,
			pm && pm->ps ? pm->ps->origin[1] : 0.0f,
			pm && pm->ps ? pm->ps->origin[2] : 0.0f,
			pm && pm->ps ? pm->ps->velocity[0] : 0.0f,
			pm && pm->ps ? pm->ps->velocity[1] : 0.0f,
			pm && pm->ps ? pm->ps->velocity[2] : 0.0f,
			pm && pm->ps ? pm->ps->pm_flags : 0,
			pm ? pm->waterlevel : 0,
			pm && pm->ps ? pm->ps->pm_type : -1);
...
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_PmoveLogBounds("ground-before-trace");
#endif
...
static void PM_SetWaterLevel( void ) {
	...
	#if defined(STEFX_SP_HOSTED_MP)
	STEFX_PmoveLogState("before-pointcontents");
	#endif
	cont = pm->pointcontents( point, pm->ps->clientNum );
	#if defined(STEFX_SP_HOSTED_MP)
	STEFX_PmoveLogState("after-pointcontents");
	#endif
...
#if defined(STEFX_SP_HOSTED_MP)
	int			stefxAmmoBefore = pm->ps ? pm->ps->ammo[WP_PHASER] : -9999;
	int			stefxRechargeBefore = pm->ps ? pm->ps->rechargeTime : -9999;
#endif
...
#if defined(STEFX_SP_HOSTED_MP)
			STEFX_PmoveLogAmmo("phaser-recharge", stefxAmmoBefore, stefxRechargeBefore);
#endif
		} 
		else 
		{
			...
			if ( pm->ps->ammo[pm->ps->weapon] < altAmmoUsage[pm->ps->weapon])
			{
				...
				if (s_stefxPmoveAmmoLogCount < 160)
				{
					XBLog_WriteCriticalf("STEFX_HM_AMMO: fire-reject-alt client=%d cmdTime=%d weapon=%d ammo=%d need=%d buttons=0x%x weapons=0x%x",
						...
						altAmmoUsage[pm->ps->weapon],
						pm->cmd.buttons,
						pm->ps->stats[STAT_WEAPONS]);
					++s_stefxPmoveAmmoLogCount;
				}
#endif
...
#if defined(STEFX_SP_HOSTED_MP)
	if ( (pm->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK)) ||
		stefxAmmoBefore != pm->ps->ammo[WP_PHASER] ||
		stefxRechargeBefore != pm->ps->rechargeTime )
	{
		STEFX_PmoveLogAmmo("post-consume", stefxAmmoBefore, stefxRechargeBefore);
	}
#endif
...
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_PmoveLogState("before-water");
#endif
	PM_SetWaterLevel();
#if defined(STEFX_SP_HOSTED_MP)
	STEFX_PmoveLogState("after-water");
#endif
	if ( !(pm->watertype & CONTENTS_LADDER) )
