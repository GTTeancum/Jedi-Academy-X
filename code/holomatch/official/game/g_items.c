...
void Add_Ammo (gentity_t *ent, int weapon, int count)
{
	...
	{
		ammoBefore = ent->client->ps.ammo[weapon];
	}
#endif
	...
	{
		XBLog_WriteCriticalf("STEFX_HM_AMMO: add client=%d weapon=%d count=%d ammo=%d->%d max=%d currentWeapon=%d weapons=0x%x",
			ent->s.number,
			weapon,
			count,
			ammoBefore,
...
/*
	...
	XBLog_WriteCriticalf("STEFX_HM_ITEM: weapon pickup client=%d weapon=%d quantity=%d ammo=%d->%d",
		...
		ammoBefore,
		other->client->ps.ammo[ent->item->giTag]);
	XBLog_WriteCriticalf("STEFX_HM_AMMO: weapon-pickup-post client=%d psWeapon=%d pickedWeapon=%d weapons=0x%x pickedAmmo=%d phaserAmmo=%d weaponstate=%d commandTime=%d",
		other->s.number,
		other->client->ps.weapon,
		ent->item->giTag,
		other->client->ps.stats[STAT_WEAPONS],
		other->client->ps.ammo[ent->item->giTag],
		other->client->ps.ammo[WP_PHASER],
		other->client->ps.weaponstate,
		other->client->ps.commandTime);
#endif
...
#if !defined(FINAL_BUILD) && !defined(STEFX_SP_HOSTED_MP)
