...
#if defined(STEFX_SP_HOSTED_MP)
static qboolean STEFX_HostedRecoverVoidBot(gentity_t *ent)
{
	char mapname[MAX_QPATH];
	static int s_stefxVoidBotLogBudget = 16;
	...
	if (ent->client->ps.origin[2] > -160.0f) {
		return qfalse;
	}

	if (s_stefxVoidBotLogBudget > 0) {
		XBLog_WriteCriticalf("STEFX_HM_BOT_RECOVER: void-fall client=%d time=%d origin=(%g,%g,%g) velocity=(%g,%g,%g) health=%d ground=%d",
			ent->s.number,
			level.time,
		...
		--s_stefxVoidBotLogBudget;
	}

	player_die(ent, NULL, NULL, 100000, MOD_FALLING);
	return qtrue;
}
#endif
...
void ClientThink_real( gentity_t *ent ) {
	...
	if (s_stefxClientThinkBoundaryTraceCount < 8)
	{
		XBLog_WriteCriticalf("STEFX_HM_SP: official ClientThink state entNumber=%d psClient=%d commandTime=%d origin=(%g,%g,%g) velocity=(%g,%g,%g) view=(%g,%g,%g) gravity=%d speed=%d pmType=%d pmFlags=0x%x",
			ent->s.number, client->ps.clientNum, client->ps.commandTime,
			...
			client->ps.gravity, client->ps.speed, client->ps.pm_type, client->ps.pm_flags);
		++s_stefxClientThinkBoundaryTraceCount;
	}
#endif
...
//		return;
	...
	{
		static int s_stefxPmoveFireLogBudget = 192;
		int stefxEventIndex;
		for ( stefxEventIndex = oldEventSequence; stefxEventIndex < client->ps.eventSequence; ++stefxEventIndex )
		{
			const int stefxEvent = client->ps.events[stefxEventIndex & (MAX_PS_EVENTS - 1)];
			if ( s_stefxPmoveFireLogBudget > 0 &&
				( stefxEvent == EV_FIRE_WEAPON || stefxEvent == EV_ALT_FIRE || stefxEvent == EV_FIRE_EMPTY_PHASER ) )
			{
				XBLog_WriteCriticalf("STEFX_WEAPON: Pmove fire event client=%d time=%d event=%d sequence=%d weapon=%d state=%d weaponTime=%d ammo=%d",
					ent->s.number,
					level.time,
					stefxEvent,
					stefxEventIndex,
					...
					client->ps.ammo[client->ps.weapon]);
				--s_stefxPmoveFireLogBudget;
			}
		}
	}
#endif
	if (IS_NAN(client->ps.origin[0]) || IS_NAN(client->ps.origin[1]) ||
		IS_NAN(client->ps.origin[2]) || IS_NAN(client->ps.velocity[0]) ||
		IS_NAN(client->ps.velocity[1]) || IS_NAN(client->ps.velocity[2]))
	{
		XBLog_WriteCriticalf("STEFX_HM_SP: Pmove produced invalid state client=%d cmdTime=%d msec=%d cmd=(%d,%d,%d) angles=(%d,%d,%d) view=(%g,%g,%g) origin=(%g,%g,%g) velocity=(%g,%g,%g)",
			ent->s.number, ucmd->serverTime, msec,
...
#if defined(STEFX_SP_HOSTED_MP)
	if (STEFX_HostedRecoverVoidBot(ent)) {
		return;
	}
#endif
	...
	{
		static int s_stefxEventCursorLogBudget = 256;
		if ( s_stefxEventCursorLogBudget > 0 )
		{
			XBLog_WriteCriticalf("STEFX_HM_EVENT_SERVER: client=%d time=%d events=%d->%d cursor=%d->%d ring=(%d,%d,%d,%d)",
				ent->s.number,
				level.time,
				oldEventSequence,
				ent->client->ps.eventSequence,
				oldEntityEventSequence,
				ent->client->ps.entityEventSequence,
				ent->client->ps.events[0], ent->client->ps.events[1],
				ent->client->ps.events[2], ent->client->ps.events[3]);
			--s_stefxEventCursorLogBudget;
		}
	}
#endif
...
void ClientThink( int clientNum ) {
	...
	if ( clientNum >= 0 && clientNum < 3 )
	{
		...
		static int s_stefxOfficialWeaponEdgeLogBudget = 192;
		const int stefxWeaponButtons = ent->client->pers.cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK);
		if ( stefxWeaponButtons != s_stefxLastOfficialWeaponButtons[clientNum] )
		{
			if ( s_stefxOfficialWeaponEdgeLogBudget > 0 )
			{
				XBLog_WriteCriticalf("STEFX_WEAPON: official ClientThink edge client=%d time=%d buttons=0x%x fire=0x%x cmdWeapon=%d psWeapon=%d state=%d weaponTime=%d",
					clientNum,
					ent->client->pers.cmd.serverTime,
					ent->client->pers.cmd.buttons,
					stefxWeaponButtons,
					ent->client->pers.cmd.weapon,
					ent->client->ps.weapon,
					ent->client->ps.weaponstate,
					ent->client->ps.weaponTime);
				--s_stefxOfficialWeaponEdgeLogBudget;
			}
			s_stefxLastOfficialWeaponButtons[clientNum] = stefxWeaponButtons;
		}
	}
#endif
