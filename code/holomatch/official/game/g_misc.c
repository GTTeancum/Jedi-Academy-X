...
static int s_stefxTeleportPlayerLogBudget = 64;
#endif
...
void TeleportPlayer( gentity_t *player, vec3_t origin, vec3_t angles, tpType_t tpType ) {
	gentity_t	*tent;
#if defined(STEFX_SP_HOSTED_MP)
	vec3_t		stefxTeleportOldOrigin;
	vec3_t		stefxTeleportOldVelocity;
	int			stefxTeleportOldFlags;
	int			stefxTeleportOldPmTime;

	VectorCopy( player->client->ps.origin, stefxTeleportOldOrigin );
	VectorCopy( player->client->ps.velocity, stefxTeleportOldVelocity );
	stefxTeleportOldFlags = player->client->ps.pm_flags;
	stefxTeleportOldPmTime = player->client->ps.pm_time;
#endif
...
#if defined(STEFX_SP_HOSTED_MP)
	if ( player->s.number == 0 && s_stefxTeleportPlayerLogBudget > 0 )
	{
		XBLog_WriteCriticalf("STEFX_HM_TELEPORT: exit client=%d type=%d oldOrigin=(%g,%g,%g) newOrigin=(%g,%g,%g) oldVelocity=(%g,%g,%g) newVelocity=(%g,%g,%g) angles=(%g,%g,%g) flags=0x%x->0x%x pmTime=%d->%d eFlags=0x%x",
			player->s.number,
			tpType,
			stefxTeleportOldOrigin[0], stefxTeleportOldOrigin[1], stefxTeleportOldOrigin[2],
			player->client->ps.origin[0], player->client->ps.origin[1], player->client->ps.origin[2],
			stefxTeleportOldVelocity[0], stefxTeleportOldVelocity[1], stefxTeleportOldVelocity[2],
			player->client->ps.velocity[0], player->client->ps.velocity[1], player->client->ps.velocity[2],
			angles[0], angles[1], angles[2],
			stefxTeleportOldFlags,
			player->client->ps.pm_flags,
			stefxTeleportOldPmTime,
			player->client->ps.pm_time,
			player->client->ps.eFlags);
		--s_stefxTeleportPlayerLogBudget;
	}
#endif
}
