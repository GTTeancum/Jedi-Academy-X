...
#define	MAX_SPAWN_POINTS	128
#if defined(STEFX_SP_HOSTED_MP)
static qboolean STEFX_HostedSpawnPointAllowed(gentity_t *spot)
{
	...
	if (!Q_stricmp(mapname, "hm_borg1") && spot->s.origin[2] < -160.0f) {
		static int s_stefxSpawnRejectBudget = 16;
		if (s_stefxSpawnRejectBudget > 0) {
			XBLog_WriteCriticalf("STEFX_HM_SPAWN: reject hosted hm_borg1 low spawn ent=%d origin=(%g,%g,%g) flags=0x%x",
				(int)(spot - g_entities),
				spot->s.origin[0], spot->s.origin[1], spot->s.origin[2],
				spot->spawnflags);
			--s_stefxSpawnRejectBudget;
		}
		return qfalse;
	}
...
gentity_t *SelectRandomDeathmatchSpawnPoint( void ) {
	...
	while ((spot = G_Find (spot, FOFS(classname), "info_player_deathmatch")) != NULL) {
#if defined(STEFX_SP_HOSTED_MP)
		if ( !STEFX_HostedSpawnPointAllowed( spot ) ) {
			continue;
		}
#endif
...
#if defined(STEFX_SP_HOSTED_MP)
static gentity_t *STEFX_SelectHostedBotSpawnPoint(gentity_t *ent, vec3_t origin, vec3_t angles)
{
	gentity_t *spot = NULL;
	gentity_t *bestSpot = NULL;
	gentity_t *bestAnySpot = NULL;
	gentity_t *localPlayer = &g_entities[0];
	vec3_t delta;
	float distanceSquared;
	float bestDistanceSquared = 999999999.0f;
	float bestAnyDistanceSquared = 999999999.0f;

	if (!ent || ent == localPlayer || !localPlayer->client ||
		localPlayer->client->pers.connected != CON_CONNECTED)
	{
		return NULL;
	}
	...
	while ((spot = G_Find(spot, FOFS(classname), "info_player_deathmatch")) != NULL)
	{
...
#endif
		if ((spot->flags & FL_NO_BOTS) || SpotWouldTelefrag(spot))
		{
			continue;
		}
		VectorSubtract(spot->s.origin, localPlayer->client->ps.origin, delta);
		...
		if (fabs(delta[2]) <= 64.0f && distanceSquared < bestDistanceSquared)
		{
	...
	if (!bestSpot)
	{
		bestSpot = bestAnySpot;
		bestDistanceSquared = bestAnyDistanceSquared;
	}
	if (!bestSpot)
	{
		return NULL;
	...
	XBLog_WriteCriticalf("STEFX_HM_SP: hosted bot spawn client=%d spot=%d distance2=%g local=(%g,%g,%g) spawn=(%g,%g,%g)",
		(int)(ent - g_entities),
		(int)(bestSpot - g_entities),
		bestDistanceSquared,
		localPlayer->client->ps.origin[0],
		localPlayer->client->ps.origin[1],
		localPlayer->client->ps.origin[2],
...
void ClientSpawn(gentity_t *ent) {
	...
	*/
	...
	if ( client->sess.sessionTeam == TEAM_SPECTATOR ) {
		...
		if (ent->r.svFlags & SVF_BOT)
		{
			spawnPoint = STEFX_SelectHostedBotSpawnPoint(ent, spawn_origin, spawn_angles);
		}
		if (!spawnPoint)
		#endif
		{
	...
	if ( ent->s.number >= 0 && ent->s.number < 3 )
	{
		...
		if ( s_stefxSpawnAmmoBudget > 0 )
		{
			XBLog_WriteCriticalf("STEFX_HM_AMMO: spawn-loadout client=%d class=%d team=%d weapon=%d weapons=0x%x phaser=%d hypo=%d recharge=%d health=%d armor=%d",
				ent->s.number,
				pClass,
				client->sess.sessionTeam,
				client->ps.weapon,
				client->ps.stats[STAT_WEAPONS],
				client->ps.ammo[WP_PHASER],
				client->ps.ammo[WP_VOYAGER_HYPO],
				client->ps.rechargeTime,
				client->ps.stats[STAT_HEALTH],
				client->ps.stats[STAT_ARMOR]);
			--s_stefxSpawnAmmoBudget;
		}
	}
#endif
	...
	if ( ent->s.number >= 0 && ent->s.number < 3 )
	{
		...
		if ( s_stefxPostThinkAmmoBudget > 0 )
		{
			XBLog_WriteCriticalf("STEFX_HM_AMMO: post-first-think client=%d time=%d weapon=%d weapons=0x%x phaser=%d hypo=%d recharge=%d flags=0x%x",
				ent->s.number,
				level.time,
				client->ps.weapon,
				client->ps.stats[STAT_WEAPONS],
	...
	if ( ent->s.number >= 0 && ent->s.number < 3 )
	{
		...
		if ( s_stefxEndFrameAmmoBudget > 0 )
		{
			XBLog_WriteCriticalf("STEFX_HM_AMMO: post-endframe client=%d time=%d weapon=%d weapons=0x%x phaser=%d hypo=%d recharge=%d flags=0x%x",
				ent->s.number,
				level.time,
				client->ps.weapon,
				client->ps.stats[STAT_WEAPONS],
	...
	#if defined(STEFX_SP_HOSTED_MP)
	XBLog_WriteCriticalf("STEFX_HM_MATCH spawn time=%d client=%d name='%s' spawnCount=%d health=%d armor=%d score=%d origin=(%g,%g,%g)",
		level.time,
		index,
		client->pers.netname,
		client->ps.persistant[PERS_SPAWN_COUNT],
