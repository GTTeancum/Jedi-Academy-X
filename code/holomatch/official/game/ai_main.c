...
static int s_botLocalTime;
static int s_botlibResidual;
static int s_lastBotThinkTime;
...
void BotUpdateInput(bot_state_t *bs, int time, int elapsed_time) {
	...
	if (bs->client > 0 && bs->client < MAX_CLIENTS) {
		int traceCount = s_stefxBotInputTraceCount[bs->client]++;
		if (traceCount < 16 || (traceCount % 100) == 0) {
			G_Printf("STEFX_HM_AI: input client=%d time=%d count=%d dir=(%g,%g,%g) speed=%g flags=0x%x weapon=%d view=(%g,%g,%g)\n",
				bs->client, time, traceCount, bi.dir[0], bi.dir[1], bi.dir[2], bi.speed,
				bi.actionflags, bi.weapon, bi.viewangles[0], bi.viewangles[1], bi.viewangles[2]);
		}
	}
#endif
	//respawn hack
...
#if defined(_XBOX) && defined(STEFX_ELITE_FORCE_MP)
	if (client > 0 && s_stefxBotAIStateLogBudget > 0) {
		G_Printf("STEFX_HM_AI: think-enter client=%d setup=%d area=%d origin=(%g,%g,%g) health=%d node=%p ltime=%g\n",
			client, bs->setupcount, bs->areanum, bs->origin[0], bs->origin[1], bs->origin[2],
			bs->cur_ps.stats[STAT_HEALTH], bs->ainode, bs->ltime);
	}
#endif
	//the real AI
...
#if defined(_XBOX) && defined(STEFX_ELITE_FORCE_MP)
	if (client > 0 && s_stefxBotAIStateLogBudget > 0) {
		G_Printf("STEFX_HM_AI: think-exit client=%d setup=%d area=%d ltg=%d enemy=%d weapon=%d node=%p stand=%g ltgTime=%g nbgTime=%g\n",
			client, bs->setupcount, bs->areanum, bs->ltgtype, bs->enemy, bs->weaponnum,
			bs->ainode, bs->stand_time, bs->ltg_time, bs->nbg_time);
		--s_stefxBotAIStateLogBudget;
	}
#endif
...
#if defined(_XBOX) && defined(STEFX_ELITE_FORCE_MP)
	G_Printf("STEFX_HM_AI: setup client=%d character=%d gs=%d ws=%d cs=%d ms=%d skill=%d enter=%g\n",
		client, bs->character, bs->gs, bs->ws, bs->cs, bs->ms, settings->skill,
		bs->entergame_time);
#endif
...
int BotAIStartFrame(int time) {
	...
	if (bot_thinktime.integer != s_lastBotThinkTime) {
		s_lastBotThinkTime = bot_thinktime.integer;
		BotScheduleBotThink();
	}

	elapsed_time = time - s_botLocalTime;
	s_botLocalTime = time;

	s_botlibResidual += elapsed_time;

	if (elapsed_time > bot_thinktime.integer) thinktime = elapsed_time;
	else thinktime = bot_thinktime.integer;

	// update the bot library
	if ( s_botlibResidual >= thinktime ) {
		s_botlibResidual -= thinktime;
...
int BotAISetup( int restart ) {
	...
	if ( !restart ) {
		...
		s_botLocalTime = 0;
		s_botlibResidual = 0;
		s_lastBotThinkTime = 0;
#if defined(_XBOX) && defined(STEFX_ELITE_FORCE_MP)
		s_stefxBotAIStateLogBudget = 160;
		memset( s_stefxBotInputTraceCount, 0, sizeof( s_stefxBotInputTraceCount ) );
#endif
		G_Printf( "STEFX_HM_SP: reset embedded bot AI statics\n" );
	}
